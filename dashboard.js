...
class Dashboard {
    ...
    switchMode(modeName) {
        ...
        modeContainers.forEach(container => container.style.display = 'none');

        // Show selected mode
        const selectedMode = document.getElementById(`${modeName}-mode`);
        if (selectedMode) {
            selectedMode.style.display = 'block';
            console.log(`Switched to mode: ${modeName}`);
        }

        // Update active button styles for tabs
        const buttons = document.querySelectorAll('.global-nav-btn');
        buttons.forEach(btn => {
            const isActive = btn.getAttribute('data-mode') === modeName;
            if (isActive) {
                btn.classList.add('border-blue-600', 'text-blue-600', 'hover:text-blue-700');
                btn.classList.remove('border-transparent', 'text-gray-600', 'hover:text-gray-900');
            } else {
                btn.classList.remove('border-blue-600', 'text-blue-600', 'hover:text-blue-700');
    ...
    switchTab(tabName) {
        ...
        // Update active button styles for segmented tabs
